...
- **4d:** DES rebuild — introduces native `split`/`concat`; validates that Feistel needs no special primitive. **DONE 2026-05-30** (shipped as scaffolding-suppression B4): each DES round is a port-mode `group` of `split-bytes`/`des.expand-R`/`des.xor-with-K`/`des.s-boxes`/`des.p-permutation`/`xor`/`concat`; the Feistel swap is the `concat` argument order; no shipped spec uses `feistel-round` anymore. `FeistelRoundGroup`/`BranchTrack`/`CombineKind` + the Feistel UI survive Phase-5-deprecation-pending. KAT byte-equal (FIPS 46-3). Obligatory follow-up: a port-native-aware Feistel/swap diagram. See `docs/plans/scaffolding-suppression.md` "## B4".
- **4d-bis (port-wiring editor): SHIPPED 2026-06-02** — plan + impl in `docs/plans/port-wiring-editor.md` (commits `b064159` headless core, `029c197` dropdown, `38ac293` canvas click-to-arm + Playwright smoke). The in-canvas affordance for the user to select a leaf and change which upstream output port each of its input ports reads from. Port wiring is no longer source-file-only. Delivered as designed below, with two refinements from the build: (a) the legal-source SET is the design spine — a scope-bounded enumerator (`src/core/port-sources.ts::legalSourcesForInput`) makes cross-scope (runtime-throwing) bindings unrepresentable by construction, while byteLength mismatches among legal sources are the only soft "coerce" case; (b) gesture = click-to-arm (user pick) with the per-input-port dropdown as the keyboard/a11y-complete fallback (and the sole path for container-seed sources the canvas draws no handle for). This sub-phase delivered:
...
- **4f:** Compose-and-save (b) — user wires N primitives, names them, saves as a new named element appearing in the palette. **Depends on 4d-bis (SHIPPED 2026-06-02) — now UNBLOCKED.**
