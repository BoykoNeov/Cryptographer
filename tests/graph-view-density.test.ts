...
describe("GraphView density — canvas size ordering", () => {
  ...
  it("normal density rounds back to the canonical 1.0× constants", () => {
    ...
    // BASE_STACK_GAP history: 6 → 12 (2026-05-19) so the four AES
    // round-body leaves got visible breathing room (pre-bump 6 px made
    // them read as one solid block), then 12 → 60 (2026-05-27, 5×) on
    // user request for "much more breathing room inside expanded
    // round/group bodies" — notably benefits SHA-256's expanded
    // message-schedule round members. Canvas auto-sizes, so the taller
    // bodies grow the SVG viewBox automatically.
    expect(c.STACK_GAP).toBe(60);
    // BASE_FLOW_GAP history: 16 → 24 (2026-05-16) for chip-row breathing
    // room (collapsed multi-block iterate UX), 24 → 36 (2026-05-19)
    // after the CBC iterate body's 13-chip row read as a wall at 24 px
    // gaps, then 36 → 72 (2026-05-27, 2×) on user request for more
    // horizontal breathing room between flowing leaves (AES rounds +
    // SHA-256 message-schedule chips benefit most). FLOW_GAP is used
