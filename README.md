...
> **Status:** v0.4.0 — multiple ciphers and modes shipped, 2D visual editor complete (all 11 slices), cross-mode mirror buttons (FIPS-197 §5.2 / §5.3.3 / Serpent per-S-box-index) across every step type with a known encrypt↔decrypt param relationship, compose-your-own block-cipher modes from the palette. See [CHANGELOG.md](./CHANGELOG.md) for the release log and [docs/versioning.md](./docs/versioning.md) for the versioning policy.
...
| `npm test` | Vitest, single run. ~1097 tests across 95 files, ~20s. |
...
tests/             ~95 files, ~1097 tests — vitest (node + jsdom mix)
