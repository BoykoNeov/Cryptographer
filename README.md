...
Padding schemes (every **block** cipher): **PKCS#7**, **zero-pad**, **ISO 7816-4**, plus a no-pad option for exact-block input. Blowfish and DES pad to their own **8-byte** block and Speck to its **4-byte** block, not 16 — the padding overlay reads the width from the active cipher. The **stream** modes (CTR, CFB, OFB) are exempt: they engage no padding at all and accept a message of any length ≥ 1 byte. ChaCha20 never pads at all.
