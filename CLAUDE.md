...
| `npm test` | Vitest, single run. Currently ~2430 tests across 214 files, ~50s total (jsdom UI tests dominate). |
| `npm run typecheck` | `tsc --noEmit`, strict. |
| `npm run check` | The gate: `biome ci . && tsc --noEmit && vitest run && vite build`. Runs in ~60s on this machine. |
...
subkeys). v1 also defers the Twofish LINEAR-view abstract diagrams (the 2-way
linear components key off `analyzeFeistelRound`, which returns null for the
4-input concat — so they simply don't render for Twofish; the per-step
PortFlowView + narrationOverride carry it).
