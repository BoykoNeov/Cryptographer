...
- Port-spreading at consumer head plan (2026-05-16 — stub, next priority): `docs/plans/port-spreading-consumer-head.md`. Source-side port-spreading shipped in commit `133676f`; 7c manual smoke surfaced fan-IN ambiguity at chip heads (collapsed-iterate chip-row consumers with heterogeneous incoming sources). Stub captures the diagnosis + approach options; flesh out before coding. Blocks Slice 7b → Feistel-plan → first Feistel cipher → universal cipher-shape.
- Universal cipher-shape plan (2026-05-16 — not yet implemented; long-horizon): `~/.claude/plans/silly-brewing-sutton.md`. Memory entry `project_universal_cipher_shape_plan.md`. `CipherShape` interface + consolidated `cipher-registry.ts` that folds today's two registration tables (`defaults` in `stores/spec.ts` + `SUPPORTED_CIPHER_MODES_BY_CIPHER` in `stores/cipher-mode.ts`) into derived data. Four phases: AES-128 refactor → AES-192/256 ECB+CBC → bytes-shape sibling steps unlock Speck/Serpent ECB+CBC → AES CTR. Hybrid type design (alias today, union later) so Feistel + stream ciphers slot in as `kind`-discriminated variants without a rename cascade. Hash/MAC/KDF/AEAD are sibling top types to `CipherShape`, not variants — they have no encrypt/decrypt symmetry. **Position in time: AFTER port-spreading at consumer head, Slice 7b, the Feistel plan, and ideally the first Feistel cipher implementation** — so the union widening absorbs real branching-state lessons rather than designing from imagination.
