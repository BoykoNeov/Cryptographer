...
- **Universal port-based dataflow plan (2026-05-21 — APPROVED after 2 advisor consults, held until DES Phase 6e complete): `docs/plans/universal-port-dataflow.md`. Memory entry `project_universal_port_dataflow_proposal.md`. Every cipher element accepts 0..N Uint8Array inputs, produces 0..N Uint8Array outputs; with explicit mismatch rules, any element can wire to any other. SUBSUMES the universal cipher-shape plan and OBVIATES the Feistel branching primitive. Design decisions locked in: Q1 hybrid (bytes + advisory layout tag), Q2 warn-and-run with deterministic right-pad/truncate-from-right coercion surfaced as visible trace step, Q3 for-each-subgraph (no canvas cycles), Q3-frame (i) one frame per (body × iteration) preserving flat trace + `:b{i}` semantics (hierarchical trace (iii) deferred as own future plan), Q-A medium canonical + fine as planned exploration construct, Q-A-parity (β) frame-equivalence at primitive boundaries + `frameMap` for compound decompositions + KAT parity at cipher boundary, Q-B both `byte-substitute` (a, first) and `lookup-substitute` (b, next) coexist, Q-C strict order (a) parameters → (b) compose-and-save → (c) TBD code/formula form. Narration preserved under rebuild via `narrationOverride: StepDocumentation` field on spec nodes (folded into Phase 1 contract design, falls back to registry doc when absent — shipped AES carries cipher-specific overrides per leaf, palette-dropped primitives get generic). Mirrors stay opt-in/suggested, never auto-applied. Feistel types fade with gradual deprecation; DES rebuild in Phase 4d uses native split/concat/xor. Six phases with explicit pass/fail gates: Phase 0 widened ~3-day trace-shape spike on `aes.sub-bytes` + `aes.add-round-key` + one ECB iteration (validates load-bearing claim across pure step + aux read + iterate boundary), Phase 1 adapter + every step lifted + `narrationOverride` field added (1–2 weeks), Phase 2 SHA-2 native (first port-native cipher), Phase 3 AES rebuild from medium primitives with `frameMap` test infrastructure, Phase 4+ rebuilds + fine primitives + compose-and-save, Phase 5 legacy deprecation. Phase 2 status (2026-05-25): Slices 2.0a/b-i/b-ii/c, 2.1a/b, 2.2, 2.3, 2.4, 2.5, 2.6a, 2.6b, 2.6c, **2.6d all GREEN**. 2.6d shipped SHA-256 decomposed into port-native primitives across 6 commits: 3 new primitives (`aux-load-bytes@1`, `byte-slice@1`, `split-bytes@1`) + a bytes-shape sibling registration `generic.state-to-aux-bytes@1` + spec rewrite using only the universal vocabulary + parity test pinning legacy ≡ decomposed ≡ node:crypto. User picks locked pre-slice (advisor consult): Q1 = (b) "W in aux entirely" (eliminates 2.6c's zero-width split-bytes + W_0..W_{t-1} loss bugs), Q2 = keep `sha2.*` helpers registered + allowlisted for saved-doc backward compat. Frame count grew 123 → 2487 (~20× pedagogy payoff — every ROTR/XOR/add-mod is visible). KAT byte-equal vs FIPS 180-4 §A.1 + node:crypto across all 6 single-block messages. Suite 2255 → 2262 (+7 in same-day advisor follow-up), bundle 627.46 KB (+13.8 KB). **Same-day 4-item follow-up landed 2026-05-25** (commit `2df4987` + docs): (1) coverage asymmetry closed — 7 dedicated `tests/state-to-aux-bytes.test.ts` tests + documented decision NOT to write legacy-vs-ported frame parity for the bytes sibling (raw layout decodes to bare Uint8Array on the ported path but BytesState on legacy — no shipped spec observes the asymmetry; aux-load-bytes is port-native and forces flag-on); (2) param-name gotcha → `docs/gotchas.md` Port-native primitives section with full table (rotate-bits-right/shift-bits-right use `bits` not `shift`, no `byteLength`; add-mod-32/xor/and/concat use `inputCount`, no `byteLength`; not@1 takes empty params; split-bytes uses `widths`) + CLAUDE.md highlights bullet + memory entry — errors NAME the missing param the executor wanted, not the wrong one supplied; (3) sequencing pin: default-collapse for the 64 round groups MUST land BEFORE Slice 2.10 (else the user sees a 1792+ chip wall on first SHA-256 render) — order is default-collapse → 2.7 → 2.10; (4) Q-A-parity β re-labelled as DELIBERATE scope cut (not "deferred") — KAT catches divergence, frameMap mostly catches structural noise. Recorded so it doesn't resurface as "2.6d wasn't finished." **Default-collapse SHIPPED 2026-05-25** (Option 1 per user pick after advisor consult — Option 5 helper-switch was rejected as a patch): per-container `defaultCollapsed?: boolean` on every container kind in `core/types.ts`, marked on SHA-256's 64 round groups; LayoutSpec gained sibling `expandedGroups?: readonly string[]` override; new pure `core/spec-defaults.ts` exposes `getDefaultCollapsedContainers` + `getEffectiveCollapsedSet` with algebra `(spec defaults ∪ collapsedGroups) − expandedGroups`; `toggleCollapse(specId, containerId, inDefaults: boolean)` signature widened, end-invariant "never in both sets" maintained, side benefit: now drops empty layouts matching `clearNodePosition` / `setReplicationMode(null)` discipline. Suite 2255 → 2285 (+30), bundle 627.46 → 628.81 KB (+1.35 KB). **Slice 2.7 SHIPPED 2026-05-25** (`requiresPortedDispatch` plumbing — user pick (b) registry-derived over (a) explicit `CipherSpec` field, keeping CipherSpec / document schemaVersion / dispatch semantics independent and avoiding a schema bump): new pure `core/dispatch.ts::requiresPortedDispatch(spec, registry): boolean` walks the tree depth-first, short-circuits on the first leaf with `kind === "ported" && legacy === undefined` (the precise runtime throw condition at `runtime.ts:607`); App.tsx threads the result into `runSpec` at line 364; `stores/spec.ts` untouched per advisor (keeps UI store out of registry-coupling). Walker descends into all five container kinds including `feistel-round.tracks[].children` — pinned by a synthetic Feistel-with-port-native-leaf test. 23-row shipped-spec table covers every cipher in `defaults` (AES/Speck/Serpent/DES → `false`, SHA-256 → `true`). Suite 2285 → 2311 (+26 = 23 specs + 3 container-descent assertions). Bundle 628.81 → 629.18 KB (+0.37 KB). SHA-256 still launchable only from tests; Slice 2.10 wires the cipher-selector entry. **Slice 2.10 sub-sliced 2026-05-25** after user picked the bigger options on both structural questions (Q-N7 = new "Hash" category over cipher-sibling, Q-N7-follow-up = hide cipher-mode for hash, Q-2.10-types = Split `Algorithm = Cipher | Hash` over extending Cipher union, Q-2.10-store = discriminated `SpecsByMode` over stub decrypt=encrypt). Split into 2.10a (type scaffolding, zero behavior change) / 2.10b (store wiring — hashDefaults + buildCanonicalPair hash branch + save schema decision) / 2.10c (UI surface + manual browser smoke + FES-with-history defaultCollapsed). **Slice 2.10a SHIPPED 2026-05-25**: `Hash = "sha-256"` + `Algorithm = Cipher | Hash` + `isHash` + `isCipher = !isHash` (structurally derived) in `stores/cipher.ts`; `SpecsByMode` widened to `CipherSpecsByMode | HashSpecsByMode` discriminated union in `stores/spec.ts`; every consumer pattern-matches on `.kind`. `buildCanonicalPair` always constructs `kind: "cipher"` (hash branch dead code in 2.10a). New `useCipherSpecsByMode()` narrowing accessor for cipher-only test files. Suite 2311 unchanged; bundle 629.18 → 630.64 KB. **Slice 2.10b SHIPPED 2026-05-25** (store wiring + save schema decision): user picks at slice start = **Option A** (widen `doc.cipher: Cipher` → `doc.algorithm: Algorithm`, bump `CipherDocument.schemaVersion` 2 → 3 with `migrateDocument` v2 → v3 step renaming the field) over Option B (sibling `hash?: Hash`) — semantically honest, scales to SHA-3/MAC/KDF with zero further schema changes; **(i) tests-only reachability** (production hash path lands in 2.10c with the algorithm selector). What shipped: `hashDefaults: Record<Hash, CipherSpec>` table (sha-256 → `buildSha256Spec()` eagerly built at module load); `HashSpecsByMode` gained a `hash: Hash` discriminant field carried forward through `updateActive` / `updateBoth` (load-bearing for `resetSpec` + `isCustomSpec` lookups — the alternative was string-matching `spec.id`); new `buildCanonicalHash(hash)` helper; `setSpecFromDocument` hash early-branch BEFORE `resolveDefault` per advisor's load-bearing gotcha (the existing cipher-axis lookup explicitly assumes a cipher universe — short-circuit is what keeps the cipher-side construction honest); `resetSpec` + `isCustomSpec` hash branches look up canonical via `current.hash`; `App.tsx` save side flips `cipher: cipher()` → `algorithm: cipher()` (Cipher is a subtype of Algorithm); load-side smart-swap guards with `isCipher(doc.algorithm)` (cipher-defaults lookup would be a category error for hash docs — 2.10c surfaces hash-input-swap when the algorithm selector exposes hash variants). Document-schema gains `HASH_IDS` const + `assertHashCoverage` compile-time check; `ALGORITHM_IDS = [...CIPHER_IDS, ...HASH_IDS]` powers `z.enum(ALGORITHM_IDS)` on the renamed field. `DEFAULT_KEY_BYTES_BY_HASH` (sha-256 → empty Uint8Array, hashes are keyless) + `DEFAULT_PT_BYTES_BY_HASH` (sha-256 → "abc" = `[0x61, 0x62, 0x63]` per FIPS 180-4 §A.1). 8 files touched (`document.ts`, `document-schema.ts`, `cipher.ts`, `spec.ts`, `App.tsx`, 5 test files). Migration test coverage: v1 → v2 → v3 chain (was v1 → v2), v2 → v3 cipher → algorithm rename (new), v2 without cipher field migrates cleanly (new), "schemaVersion 4 unsupported" replaces former "schemaVersion 3 unsupported". `tests/spec-store-hash-branch.test.ts` (NEW, 7 tests) walks the hash path via `setSpecFromDocument` — same boundary file-load + URL-share use, so when 2.10c wires the dropdown the store contract is already exercised end-to-end. Suite 2311 → **2321** (+10 = 7 hash-branch + 2 new migration + 1 hash variant in algorithm enum), bundle 630.64 → 640.21 KB raw (+9.57 KB — migration helper + eager `hashDefaults` build of the 1800-leaf SHA-256 spec at module load is the dominant contributor). Schema bump SCHEMA_VERSION 2 → 3 is the third in project history (1 → 2 for DES/feistel-round; 2 → 3 for cipher → algorithm rename). Next: **2.10c** (UI surface — algorithm selector dropdown adopting Cipher | Hash categories, hide cipher-mode for hash, FES-with-history `msg-schedule` `defaultCollapsed: true` gotcha pre-empted, plaintext validator hash branch, manual browser smoke). `narrationOverride` content lands in Slice 2.8.**
