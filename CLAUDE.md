...
- **Universal port-based dataflow proposal (2026-05-21 — PROPOSAL, advisor FIRST next session): `docs/plans/universal-port-dataflow.md`. Memory entry `project_universal_port_dataflow_proposal.md`. User idea: every cipher element accepts 0..N Uint8Array inputs, produces 0..N Uint8Array outputs; with explicit mismatch rules, any element can wire to any other. SUBSUMES the universal cipher-shape plan and OBVIATES the Feistel branching primitive (those exist only because the current single-state-thread + aux model can't express branching/multi-output natively). Three open design questions captured: Q1 port shape labels (bytes-only vs hybrid layout tag), Q2 mismatch behavior (coerce / refuse / warn-and-run — recommended warn-and-run), Q3 aux feedback loops (CBC IV chain). Recommended migration: incremental dual-contract — `PortedExecutor` alongside today's `StepExecutor`, adapter `liftLegacyExecutor()` for all shipped steps, new ciphers (first SHA-2 candidate) lead with unified shape. Next-session hand-off: call `advisor()` with the full plan doc FIRST THING before any code work.**
- Universal cipher-shape plan (2026-05-16 — not yet implemented; **likely subsumed by universal-port-dataflow proposal**, but kept here pending advisor + user decision): `~/.claude/plans/silly-brewing-sutton.md`. Memory entry `project_universal_cipher_shape_plan.md`. `CipherShape` interface + consolidated `cipher-registry.ts` that folds today's two registration tables (`defaults` in `stores/spec.ts` + `SUPPORTED_CIPHER_MODES_BY_CIPHER` in `stores/cipher-mode.ts`) into derived data. Four phases: AES-128 refactor → AES-192/256 ECB+CBC → bytes-shape sibling steps unlock Speck/Serpent ECB+CBC → AES CTR. Hybrid type design (alias today, union later) so Feistel + stream ciphers slot in as `kind`-discriminated variants without a rename cascade. Hash/MAC/KDF/AEAD are sibling top types to `CipherShape`, not variants — they have no encrypt/decrypt symmetry. **Position in time: AFTER the Feistel plan and ideally the first Feistel cipher implementation** (port-spreading + Slice 7b already shipped) — so the union widening absorbs real branching-state lessons rather than designing from imagination.
