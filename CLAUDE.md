...
- solid-flow spike for Slice 8 mechanic (in progress on branch `explore/solid-flow`, dev server on port 5174): `docs/plans/solid-flow-spike.md`. Evaluates whether a graph framework (solid-flow, since `@xyflow/solid` doesn't exist and the React xyflow path was rejected on bundle grounds) should host palette-drop step insertion, or whether Slice 8 should land via HTML5 DnD on the existing SVG. Adds a fourth `view-mode` so both views coexist for side-by-side comparison.
