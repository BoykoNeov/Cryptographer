...
- DES + branching primitive plan (Phase 6 complete; legacy contract): `docs/plans/des-feistel.md`. Memory: `project_des_feistel_plan.md`. First Feistel cipher; introduces `FeistelRoundGroup` + `tracks: BranchTrack[]` + named `combineKind`. Schema bump 1→2 in phase 2. Universal-port-dataflow plan Phase 4d will rebuild DES from native primitives and fade `FeistelRoundGroup`.
...
- SHA-256 density polish plan (2026-05-26 — Slice 2.8 follow-up; S1 shipped 2026-05-26): `docs/plans/sha-256-density-polish.md`. Memory: `project_sha_256_density_polish_plan.md`. Three sub-slices — S1 ParamEditor blocks for port-native primitives (SHIPPED), S2 graph preamble-row layout fixes, S3 narration density second pass.
- Gradual obstacle-aware edge routing plan (2026-05-27 — APPROVED, NOT STARTED): `docs/plans/gradual-obstacle-aware-routing.md`. Memory: `project_gradual_obstacle_routing_plan.md`. Third take after `explore/altitude-staggering` + `explore/edge-routing-router` neither merged. Six slices — Slice 0 probes (cluster cardinality + multi-leaf crossing + 8 px inflation feasibility + CBC feedback-gap), Slices 1-2 pure (clustering + A* midline + Catmull-Rom smoothing + endpoint flares), Slice 3 wire-up + smoke + diagnostic hatch, Slices 4-5 optional polish (compose altitude + click-disambiguate via S2(m)). Architectural decision: subsume the `isFeedback` geom branch (`EdgePath.geom()` lines 7276-7357) into unified routing via A\* directional bias — `isFeedback` survives as semantic flag (dashed style + tooltip + cycle-detection exclusion) but stops controlling geometry. Two-tier replica obstacle treatment: chips hard, arrow corridors soft. Cross-plan handoff: SHA-256 W_{t-2/7/15/16} cross-iteration visibility is a derivation gap belonging in the universal-port-dataflow plan family (extend `inferHistorySeedEdges` to synthesize ~128 cross-iteration edges); THIS plan's routing then renders them as bundles automatically.
