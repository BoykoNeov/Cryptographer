...
- solid-flow spike for Slice 8 mechanic — **concluded with Abandon recommendation** (branch `explore/solid-flow`, writeup at `docs/plans/solid-flow-spike.md`). Findings: `solid-flow@1.0.4`'s shipped `.d.ts` lacks subflow nesting (`NodeProps` has no `parentId`/`type`), custom edge types (`EdgeProps` has no `type`/`data`/styling), and a drop handler — every Phase 1 success criterion was blocked by API surface, so no adapter was written. Slice 8 ships as **HTML5 DnD on the existing SVG `GraphView.tsx`**: `dragover`/`drop` on the root `<svg>` + `getScreenCTM().inverse()` for canvas coords + the already-shipped `core/spec-mutations.ts::insertStepAfter`. Optional `@thisbeyond/solid-dnd` (~5 KB) only if native event ergonomics feel rough.
