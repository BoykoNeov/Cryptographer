...
- Graph framework spike — **concluded with Don't-Merge recommendation** (branch `explore/solid-flow`, writeup at `docs/plans/solid-flow-spike.md`). Two paths evaluated: (1) `solid-flow@1.0.4` abandoned at typing survey — no subflow nesting / custom edges / drop handler. (2) `@xyflow/react@12` via React-in-Solid interop — Phase 1 built end-to-end (adapter + Solid wrapper + view-mode tab); Phase 2 ported the SVG view's readability tricks (collapse, high-fanout replication, view-density scaling, drag-to-pin) so the comparison is apples-to-apples. **Disqualified on bundle: +121.68 KB gzipped over the 76 KB baseline.** Branch kept alive as a working reference if the project ever pivots to a scope where the bundle delta is amortized; the adapter + wrapper + transform pipeline are all functional. Slice 8 was shipped on `main` independently as HTML5 DnD on the SVG view.
